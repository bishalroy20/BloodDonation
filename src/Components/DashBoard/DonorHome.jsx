    ...
    <div className="block sm:hidden space-y-4">
      {recent.map((r) => (
        <div key={r._id} className="border rounded p-3 space-y-1">
          ...
          <p><span className="font-semibold">Donor Info:</span> {r.status === "inprogress" ? `${r.donorName} (${r.donorEmail})` : "—"}</p>
          <div className="flex flex-col gap-2 mt-2">
            ...
            <Link to={`/dashboard/requests/${r._id}/edit`} className="text-center px-3 py-1 bg-blue-600 text-white rounded">
              Edit
            </Link>
            <button
            ...
            <Link to={`/requests/${r._id}`} className="text-center px-3 py-1 bg-black text-white rounded">
              View
            </Link>
          </div>
        </div>
